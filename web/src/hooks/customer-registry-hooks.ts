...
import { invoicingClient, type CustomerInput } from "../lib/invoicing-client.ts"
import type { Customer, PartyType } from "../lib/models.ts"
import { usePagedList } from "./paged-query.ts"
import { countyRequiresSector } from "../lib/romanian-counties.ts"
import { normalizeRomanianCui } from "../lib/vat-defaults.ts"
...
export const useCustomerRegistry = (notify: (message: string) => void) => {
  ...
  const submit = (event: FormSubmitEvent): void => {
    ...
    if (fiscalIdentifier instanceof HTMLInputElement) {
      const missingCompanyCui = partyType === "company" && fiscalIdentifier.value.trim() === ""
      fiscalIdentifier.setCustomValidity(missingCompanyCui ? "CUI / CIF este obligatoriu pentru persoanele juridice." : "")
      if (missingCompanyCui) { fiscalIdentifier.reportValidity(); fiscalIdentifier.setCustomValidity(""); return }
    }
