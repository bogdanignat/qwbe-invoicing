...
import type { Issuer, Proforma } from "../../lib/models.ts"
...
const issuer: Issuer = {
  organizationId: "org-1", name: "Cabinet Individual Exemplu", fiscalIdentifier: "2", legalForm: "pfa", tradeRegistryNumber: "F1234567890123",
  iban: "RO49AAAA1B31007593840000", bankName: "Banca Exemplu cu denumire suficient de lungă", socialCapital: "",
  address: { countryCode: "RO", city: "Botoșani", street: longToken, county: "RO-BT", postalCode: "710001" },
...
void test("shares compact, complete issuer presentation between authoring and readonly documents", async (context) => {
  ...
  const sellerModule = await server.ssrLoadModule("/src/components/document/SellerSummary.tsx") as {
  ...
  for (const value of ["Cabinet Individual Exemplu", "MARCA EMITENTULUI", "PFA", "CUI / CIF: 2", "F1234567890123", "RO49AAAA1B31007593840000", "Banca Exemplu", longToken, "Botoșani", "710001", "RO"]) assert.match(seller, new RegExp(value))
  ...
  const snapshot: Snapshot = {
    currency: "RON", issueDate: "2026-09-11", dueDate: null, notes: null, issuer: { ...issuer, vatRegistered: true },
  ...
  assert.match(document, /Cod TVA: RO2/)
  assert.match(document, /Cod TVA: RO1/)
