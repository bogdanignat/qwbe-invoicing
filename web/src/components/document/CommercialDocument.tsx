...
import { buyerFiscalIdentity, formattedRomanianAddress } from "../../lib/fiscal-identity.ts"
import type { Proforma } from "../../lib/models.ts"
...
export const CommercialDocument = ({ snapshot, identity, lineCaption }: CommercialDocumentProps) => {
  ...
  />
  <div className="table-wrap"><table><caption className="sr-only">{lineCaption}</caption><thead><tr><th>Descriere</th><th>Cantitate</th><th>U.M.</th><th>Preț unitar</th><th>TVA</th><th>Total</th></tr></thead><tbody>{snapshot.lines.map((line) => <tr key={line.id}><td>{line.description}</td><td>{line.quantity}</td><td>{line.unitOfMeasure.name} — {line.unitOfMeasure.code}</td><td>{money(line.unitPrice, snapshot.currency)}</td><td>{vatTreatmentLabel(line)}</td><td>{money(line.totalIncludingVat, snapshot.currency)}</td></tr>)}</tbody></table></div>
  <div className="invoice-bottom"><div className="document-tax-details"><h3>Detaliu TVA</h3>{snapshot.vatBreakdown.map((tax) => <p key={`${tax.vatCategoryCode}-${tax.rate}`}>{vatTreatmentLabel(tax)} · bază {money(tax.vatBaseAmount, snapshot.currency)} · TVA {money(tax.vatAmount, snapshot.currency)}{tax.vatExemptionReason === null ? null : <> · {tax.vatExemptionReason}</>}</p>)}</div><dl><div><dt>Subtotal</dt><dd>{money(snapshot.totalExcludingVat, snapshot.currency)}</dd></div><div><dt>TVA</dt><dd>{money(snapshot.vatTotal, snapshot.currency)}</dd></div><div className="grand-total"><dt>Total</dt><dd>{money(snapshot.totalIncludingVat, snapshot.currency)}</dd></div></dl></div>
