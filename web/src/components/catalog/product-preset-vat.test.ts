...
import type { EditableInvoiceLine } from "../../lib/invoice-authoring-state.ts"
import { ARTICLE_310_EXEMPTION_REASON, type Issuer, type ProductPreset, type VatCatalogue, type VatRate } from "../../lib/models.ts"
import { preferableVatRates, presetVatIssue, presetVatLabel, presetVatOptions } from "../../lib/product-preset-vat.ts"
import type { ProformaAuthoringSessionInput } from "../../hooks/proforma-authoring-hooks.ts"
...
void test("both authoring forms resolve a chosen product's code on the form's issue date", async (context) => {
  ...
  const invoices = await server.ssrLoadModule("/src/hooks/invoice-authoring-session-hooks.ts") as {
    readonly useInvoiceAuthoringSession: (input: InvoiceAuthoringSessionInput) => AuthoringProbe
  }
  const proformas = await server.ssrLoadModule("/src/hooks/proforma-authoring-hooks.ts") as {
...
void test("renders an expired preference or choice as its own option with a blocking warning", async (context) => {
  const server = await createServer({ appType: "custom", server: { middlewareMode: true, hmr: false } })
  context.after(async () => { await server.close() })
  const module = await server.ssrLoadModule("/src/components/catalog/PresetVatField.tsx") as {
