...
export const InvoiceDocument = ({ invoice }: { readonly invoice: IssuedInvoice }) => <div className="invoice-document card">
  <div className="invoice-parties"><section><p className="eyebrow">Furnizor</p><h2>{invoice.issuer.legalName}</h2><p>{invoice.issuer.taxIdentifier}<br />{invoice.issuer.address.street}, {invoice.issuer.address.city}</p></section><section><p className="eyebrow">Client</p><h2>{invoice.customer.legalName}</h2><p>{invoice.customer.taxIdentifier}<br />{invoice.customer.address.street}, {invoice.customer.address.city}</p></section></div>
