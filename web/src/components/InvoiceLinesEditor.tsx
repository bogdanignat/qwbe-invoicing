...
import type { TaxConfiguration } from "../models.ts"
...
export const InvoiceLinesEditor = ({ lines, taxConfigurations, issueDate, pending, onAdd, onChange, onDelete }: InvoiceLinesEditorProps) => {
  ...
  return <section className="card authoring-section">
    <div className="section-heading"><div><h2>4. Produse și servicii</h2><p>Introdu manual orice produs sau serviciu. Linia nu trebuie să existe într-un catalog.</p></div><button className="button secondary small" type="button" disabled={pending} onClick={onAdd}>Adaugă linie</button></div>
