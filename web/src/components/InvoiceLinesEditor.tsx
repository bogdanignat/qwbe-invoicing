...
import type { ProductPreset, VatConfiguration, UnitOfMeasure } from "../models.ts"
...
export const InvoiceLinesEditor = ({ lines, productPresets, vatConfigurations, unitOfMeasures, issueDate, pending, onAdd, onChange, onApplyPreset, onDelete }: InvoiceLinesEditorProps) => {
  const taxes = vatConfigurations.filter((tax) => tax.effectiveFrom <= issueDate && (tax.effectiveTo === undefined || issueDate <= tax.effectiveTo))
