...
export const CommercialDocument = ({ snapshot, heading, notice, lineCaption }: CommercialDocumentProps) => <div className="invoice-document card">
  ...
  <div className="table-wrap"><table><caption className="sr-only">{lineCaption}</caption><thead><tr><th>Descriere</th><th>Cantitate</th><th>U.M.</th><th>Preț unitar</th><th>TVA</th><th>Total</th></tr></thead><tbody>{snapshot.lines.map((line) => <tr key={line.id}><td>{line.description}</td><td>{line.quantity}</td><td>{line.unitOfMeasure.name} — {line.unitOfMeasure.code}</td><td>{money(line.unitPrice, snapshot.currency)}</td><td>{line.taxRate}%</td><td>{money(line.totalIncludingTax, snapshot.currency)}</td></tr>)}</tbody></table></div>
