...
export const CommercialDocument = ({ snapshot, heading, notice, lineCaption }: CommercialDocumentProps) => <div className="invoice-document card">
  ...
  <div className="invoice-parties"><section><p className="eyebrow">Furnizor</p><IssuerBrand branding={snapshot.issuer.branding} /><h2>{snapshot.issuer.name}</h2><p>{snapshot.issuer.fiscalIdentifier === "" ? null : <>CUI / CIF: {snapshot.issuer.fiscalIdentifier}<br /></>}{address(snapshot.issuer.address)}</p></section><section><p className="eyebrow">Cumpărător · {snapshot.customer.partyType === "company" ? "PJ" : "PF"}</p><h2>{snapshot.customer.name}</h2><p>{snapshot.customer.fiscalIdentifier === "" ? null : <>{identifierLabel(snapshot.customer.partyType)}: {snapshot.customer.fiscalIdentifier}<br /></>}{address(snapshot.customer.address)}</p></section></div>
