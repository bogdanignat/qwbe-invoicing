  ...
  return <Page title="Clienți" eyebrow="Registru activ" actions={<a className="button primary" href="/invoices/new">Factură nouă</a>}>
    <div className="split-layout">
      ...
      <section className="card sticky-card"><h2 ref={editHeading} tabIndex={-1} aria-live="polite">{editing === undefined ? "Client nou" : "Editează clientul"}</h2><p>Termenul de plată este opțional; dacă lipsește, se folosește valoarea din setările firmei.</p>{state.save.error === null ? null : <ErrorAlert error={state.save.error} />}<form key={editing?.id ?? "new"} onSubmit={state.submit}>
        ...
        <label>{state.partyType === "company" ? "Denumire" : "Nume complet"}<input name="legalName" required defaultValue={editing?.legalName ?? ""} /></label>
        <label>{identifierLabel(state.partyType)} {state.partyType === "individual" ? <span className="optional">opțional</span> : null}<input name="taxIdentifier" defaultValue={editing?.taxIdentifier ?? ""} pattern={state.partyType === "company" ? romanianCuiPattern : "(?:[0-9]{13})?"} maxLength={13} title={state.partyType === "company" ? "CUI românesc valid, cu sau fără prefixul RO" : "CNP valid din 13 cifre sau câmp gol"} inputMode={state.partyType === "individual" ? "numeric" : "text"} onInput={(event) => { event.currentTarget.value = state.partyType === "company" ? normalizeRomanianCui(event.currentTarget.value) : event.currentTarget.value.replace(/\D/g, "") }} aria-describedby="customer-identifier-hint" /></label>
        ...
        <div className="form-actions">{editing === undefined ? null : <button className="button ghost" type="button" disabled={state.save.isPending} onClick={state.cancelEdit}>Renunță</button>}<button className="button primary" type="submit" disabled={state.save.isPending}>{state.save.isPending ? "Se salvează…" : editing === undefined ? "Adaugă client" : "Salvează modificările"}</button></div>
      </form></section>
    </div>
  </Page>
}
