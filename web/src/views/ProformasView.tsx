...
export const ProformasView = () => {
  ...
  return <Page title="Proforme" eyebrow="Documente comerciale" actions={<ButtonLink href="/invoices/new">Document nou</ButtonLink>}>
    ...
    <section className="card">
      ...
      {proformas.data.length === 0 ? <EmptyState>Nu există încă proforme emise.</EmptyState> : <div className="table-wrap"><table><caption className="sr-only">Registru de proforme</caption><thead><tr><th>Număr</th><th>Client</th><th>Emisă</th><th>Scadență</th><th>Total</th><th>Status</th></tr></thead><tbody>{proformas.data.map((proforma) => { const status = proformaStatusPresentation(proforma); return <tr key={proforma.id} className="clickable-row"><td><a href={`/proformas/${encodeURIComponent(proforma.id)}`}><strong>{proforma.series} {proforma.number}</strong></a></td><td>{proforma.customer.legalName}</td><td>{proforma.issueDate}</td><td>{proforma.dueDate ?? "—"}</td><td>{money(proforma.totalIncludingTax, proforma.currency)}</td><td><span className={`badge ${status.tone}`}>{status.label}</span></td></tr> })}</tbody></table></div>}
    </section>
  </Page>
}
