  ...
  return <Page title="Draft factură" eyebrow={`Scadență ${draft.data.dueDate}`}>
    <div className="split-layout invoice-layout">
      <section className="card">
        ...
        <form className="inline-form" onSubmit={submit}><label>Descriere<input name="description" required /></label><label>Cantitate<input name="quantity" inputMode="decimal" defaultValue="1" required /></label><label>Preț unitar<input name="unitPrice" inputMode="decimal" required /></label><label>Cod TVA<input name="taxCode" defaultValue="RO_STANDARD" required /></label><button className="button secondary" type="submit" disabled={addLine.isPending}>{addLine.isPending ? "Se adaugă…" : "Adaugă linia"}</button></form>
      </section>
      <aside className="card sticky-card summary-card"><h2>Sumar draft</h2>{issue.error === null ? null : <ErrorAlert error={issue.error} />}<dl><div><dt>Data emiterii</dt><dd>{draft.data.issueDate}</dd></div><div><dt>Scadență</dt><dd>{draft.data.dueDate}</dd></div><div><dt>Monedă</dt><dd>{draft.data.currency}</dd></div><div><dt>Linii</dt><dd>{draft.data.lines.length}</dd></div></dl><button className="button primary wide" type="button" onClick={() => { if (window.confirm("Emiți factura? Seria, numărul și snapshot-ul fiscal devin imuabile.")) issue.mutate() }} disabled={draft.data.lines.length === 0 || issue.isPending}>{issue.isPending ? "Se emite…" : "Emite factura"}</button><p className="hint">Emiterea alocă seria și numărul fiscal.</p></aside>
    </div>
  </Page>
}
