...
import { nameKeyset, optionalText, read, row, rowsWanted, text, write, type Row } from "./sqlite-rows.ts"
...
export const catalogTransactionAdapter = (database: DatabaseSync): CatalogTransaction => ({
  saveProductPreset: (preset) => write("save product preset", () => {
    const result = database.prepare(`INSERT INTO product_presets(id,organization_id,description,unit_price,unit_code,unit_name,preferred_vat_rate_code)
      VALUES(?,?,?,?,?,?,?)
      ON CONFLICT(id) DO UPDATE SET description=excluded.description,unit_price=excluded.unit_price,
       unit_code=excluded.unit_code,unit_name=excluded.unit_name,preferred_vat_rate_code=excluded.preferred_vat_rate_code
