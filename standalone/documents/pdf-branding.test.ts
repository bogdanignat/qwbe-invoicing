...
const invoice: RenderableInvoiceFixture = {
  ...
  lines: [{ description: "Servicii", quantity: "1", unitPrice: "100", unitOfMeasure: { code: "HUR", name: "oră" },
    vatRate: "21", vatCategoryCode: "S", vatExemptionReason: null,
    totalExcludingVat: "100", vatAmount: "21", totalIncludingVat: "121" }],
  vatBreakdown: [{ rate: "21", vatCategoryCode: "S", vatExemptionReason: null,
    vatBaseAmount: "100", vatAmount: "21" }], totalExcludingVat: "100", vatTotal: "21", totalIncludingVat: "121",
}
...
void test("renders none, custom text, image and both deterministically in invoice and proforma PDFs", async () => {
  ...
  for (const branding of [null, { text, image: null }, { text: null, image }, { text, image }]) {
    ...
    for (const [render, version] of [[() => renderer.render(source), "invoice-v9"], [() => renderer.renderProforma(proforma), "proforma-v8"]] as const) {
