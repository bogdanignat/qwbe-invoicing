...
void test("persists an issued snapshot across store recreation and isolates organizations", async () => {
  ...
  try {
    ...
    try {
      ...
      assert.throws(() => database.prepare("UPDATE issued_invoices SET actor_id = 'someone-else' WHERE id = ?").run(issued.id))
      assert.equal(database.prepare("SELECT actor_id FROM issued_invoices WHERE id = ?").get(issued.id)?.actor_id, "user-1")
      const trail = database.prepare("SELECT actor_id, action, target_kind, target_id FROM audit_events WHERE target_id = ?").all(issued.id).map((row) => ({ ...row }))
      assert.deepEqual(trail, [{ actor_id: "user-1", action: "invoice.issued", target_kind: "invoice", target_id: issued.id }])
      assert.throws(() => database.prepare("UPDATE audit_events SET actor_id = 'someone-else' WHERE target_id = ?").run(issued.id))
      assert.throws(() => database.prepare("DELETE FROM audit_events WHERE target_id = ?").run(issued.id))
