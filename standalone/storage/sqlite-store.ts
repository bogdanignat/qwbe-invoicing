...
type ProformaWorkflowTransaction = InvoicingTransaction & IssuerTransaction & ProformaTransaction & CorrectionsTransaction & {
...
const brandingFrom = (value: Row, field: string): IssuerBranding | null => {
  ...
  if (!(candidate.text === null || typeof candidate.text === "string")
    || !(image === null || (typeof image === "object" && !Array.isArray(image)
      ...
      && typeof (image as Readonly<Record<string, unknown>>).height === "number"))) {
    throw new Error(`invalid ${field}`)
  }
  return branding as IssuerBranding
}
...
const storedVatRates = new Map<string, ReadonlySet<string>>([
  ["RO_STANDARD", new Set(["19.00", "21.00"])],
  ["RO_REDUCED", new Set(["9.00", "11.00"])],
  ["RO_REDUCED_5", new Set(["5.00"])],
  ["RO_NON_VAT", new Set(["0.00"])],
])

const vatTreatment = (code: string, rate: string, category: string, reason: string | null) => {
...
const vatTreatmentFrom = (value: Row, codeField: string, rateField: string, categoryField: string) => {
...
const issuedInvoiceFrom = (database: DatabaseSync, value: Row): IssuedInvoice => {
  ...
  return {
    ...
    issuer: issuerFrom(value, "issuer_branding"), customer: buyerFrom(value, "customer_"),
...
const proformaFrom = (database: DatabaseSync, value: Row): Proforma => {
  ...
  return {
    ...
    dueDate: nullableText(value, "due_date"), issuedAt: text(value, "issued_at"), actorId: text(value, "actor_id"), currency: text(value, "currency"),
    notes: nullableText(value, "notes"),
    issuer: issuerFrom(value, "issuer_branding"), customer: buyerFrom(value, "customer_"),
...
const draftFrom = (database: DatabaseSync, value: Row): DraftInvoice & { readonly sourceProformaId: string | null } => {
  const id = text(value, "id")
  ...
  return {
    ...
    currency: text(value, "currency"), notes: nullableText(value, "notes"), status: draftStatus(value),
    lines, ...calculateTotals(lines),
  }
}

const appendAuditEvent = (database: DatabaseSync, event: AuditEvent): void => {
...
const transactionAdapter = (database: DatabaseSync): ProformaWorkflowTransaction => ({
    ...
    database.prepare(`INSERT INTO issued_invoices
      ...
      .run(invoice.id, invoice.draftId, invoice.sourceProformaId, invoice.organizationId, ...sourceValues(invoice.source), Number(invoice.issueDate.slice(0, 4)),
        invoice.series, invoice.number, invoice.issueDate, invoice.dueDate, invoice.issuedAt, invoice.currency,
         ...
         Number(invoice.customer.vatRegistered),
    ...
    database.prepare(`INSERT INTO proformas
      ...
      .run(proforma.id, proforma.sourceDraftId, proforma.organizationId, ...sourceValues(proforma.source), Number(proforma.issueDate.slice(0, 4)), "proforma",
        proforma.series, proforma.number, proforma.issueDate, proforma.dueDate, proforma.issuedAt, proforma.currency,
         ...
         Number(proforma.customer.vatRegistered),
        proforma.totalExcludingVat, proforma.vatTotal, proforma.totalIncludingVat, proforma.notes, proforma.actorId)
  ...
  saveCorrection: (correction) => write("save correction", () => {
    database.prepare(`INSERT INTO correction_documents
      ...
      .run(correction.id, correction.organizationId, ...sourceValues(correction.source), correction.originalInvoiceId, correction.fiscalYear,
        correction.series, correction.number, correction.issueDate, correction.issuedAt, correction.reason, correction.currency,
         ...
         Number(correction.customer.vatRegistered),
    ...
    const lineStmt = database.prepare(`INSERT INTO correction_lines
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    correction.lines.forEach((line, pos) => {
      vatTreatment(line.vatRateCode, line.vatRate, line.vatCategoryCode, line.vatExemptionReason)
      lineStmt.run(line.id, correction.id, pos, line.description, line.quantity, line.unitPrice,
    ...
    correction.vatBreakdown.forEach((tax, pos) => {
      vatTreatment(tax.code, tax.rate, tax.vatCategoryCode, tax.vatExemptionReason)
      taxStmt.run(correction.id, pos, tax.code, tax.vatCategoryCode, tax.rate, tax.vatExemptionReason,
        tax.vatBaseAmount, tax.vatAmount)
    })
  }),
  findCorrection: (organizationId, id) => read("find correction", () => {
    ...
    return {
      ...
      actorId: text(value, "actor_id"), currency: text(value, "currency"),
...
const paymentsTransactionAdapter = (database: DatabaseSync): PaymentsTransaction => ({
  ...
  appendAuditEvent: (event) => paymentWrite("append audit event", () => { appendAuditEvent(database, event) }),
})
...
export const createSqliteStore = (dataDirectory: string): TransactionalStore<InvoicingTransaction & CustomersTransaction & CatalogTransaction & IssuerTransaction & ProformaTransaction & CorrectionsTransaction> => ({
