...
void test("the SPA route table and the static host agree on every route", () => {
  ...
  assert.deepEqual(matchUiRoute("/proformas/new"), { kind: "proforma-new" })
  ...
  for (const path of ["/", "/app", "/unlock", "/invoices", "/invoices/new", "/invoices/x", "/proformas", "/proformas/new", "/proformas/x", "/drafts/x", "/customers", "/products", "/settings"]) {
